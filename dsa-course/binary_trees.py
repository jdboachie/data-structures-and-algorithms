...
class BinaryTree(object):
    ...
    def levelorder_print(self, start: Node) -> str:
        if start is None:
            return
        ...
        traversal = ""
        while queue.qsize() > 0:
            traversal += str(queue.queue[0].value) + "-"
            node = queue.get()
    ...
    def reverse_levelorder_print(self, start: Node) -> str:
        if start is None:
            return
    ...
    def height(self, node):
        ...
        def calculate_height(node):
            ...
            # Memoize the result
            memo[node] = 1 + max(left_height, right_height)
